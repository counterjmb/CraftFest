...
/*
 * Used Essence Mod for reference 
 */
...
public class ModTeleporter extends Teleporter {
	...
	@Override
	public void placeInPortal(Entity entity, float rotaionYaw) {
		if (this.worldServerInstance.provider.getDimensionId() != 0) {
			if (!this.placeInExistingPortal(entity, rotaionYaw)) {
				this.makePortal(entity);
				this.placeInExistingPortal(entity, rotaionYaw);
			}
		} 
	}
	...
	public boolean placeInExistingPortal(Entity entity, float yaw) {
		...
		if (d0 >= 0.0D)
		{
			...
			if (enumfacing != null)
			{
				...
				entity.rotationYaw = yaw - (float)(enumfacing1.getHorizontalIndex() * 90) + (float)(enumfacing.getHorizontalIndex() * 90);
			}
			else
			{
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						flag = k3 < 0;
						this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? frameBlock.getDefaultState() : Blocks.air.getDefaultState());
					}
				}
			}
		}
		...
		for (i3 = 0; i3 < 4; ++i3) {
			for (j3 = 0; j3 < 4; ++j3) {
				for (k3 = -1; k3 < 4; ++k3) {
					...
					this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), (flag ? frameBlock.getDefaultState() : portal.getDefaultState()), 2);
				}
			}
